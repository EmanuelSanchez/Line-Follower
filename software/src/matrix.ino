...
void print_bool_matrix(){
  Serial.println("Matrix:");
  for(int i=0; i<7; i++){
    for(int j=0; j<7; j++){
      Serial.print(Bool_Matrix[i][j]);
      Serial.print("\t");
    }
  Serial.println();
  }
...
float getLineCenterofMassAnalog(int line){
  float center;
  float weights;
  center = 0;
  weights = 0;
  for (int i=0; i<7; i++){
    center += Matrix_Result[line][i]*(i-3);
    weights += Matrix_Result[line][i];
  }
...
float getMatrixPromCenterMass(void){
  float center_mass;
  center_mass = 0;
  for(int j=0; j<4; j++){
    center_mass += getLineCenterofMassBool(j);
  }
  center_mass = center_mass/7;
  Serial.print("\nCenter: ");
  Serial.print(center_mass);
  return center_mass;
}

float getLineCenterofMassBool(int line){
  float center;
  float weights;
  center = 0;
  weights = 0;
  for (int i=0; i<7; i++){
    center += Bool_Matrix[line][i]*(i-3);
    weights += Bool_Matrix[line][i];
  }
  center = center/weights;
  return center;
}
